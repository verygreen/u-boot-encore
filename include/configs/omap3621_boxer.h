...
#define CONFIG_3621BOXER	1    /* for specific IEC configuration	*/
#define	CONFIG_SDRAM_ELPIDA_EDD20323ABH	1
...
#define CONFIG_COMMANDS		((CFG_CMD_I2C | CONFIG_CMD_DFL | \
				  CFG_CMD_FAT | CFG_CMD_EXT2 | CFG_CMD_MMC |\
				  CFG_CMD_NAND) & ~(C_MSK))
...
#define CONFIG_BOOTCOMMAND "run autodetectmmc; run setserialnum; run incbootcount; run checkbcb; run ${bootvar}"
...
setenv bootargs console=ttyS0,115200n8 androidboot.console=ttyS0 initrd rw init=/init videoout=omap24xxvout omap_vout.video1_numbuffers=6 omap_vout.vid1_static_vrfb_alloc=y omapfb.vram=0:5M;\
mmcinit $mmcbootdev; fatload mmc $mmcbootdev 0x81c00000 uRecoveryImage; fatload mmc $mmcbootdev 0x81f00000 uRecoveryRamdisk; bootm 0x81c00000 0x81f00000" \
	"\0" \
\
"normalboot=\
...
mmcinit $mmcbootdev; fatload mmc $mmcbootdev 0x81c00000 uImage; fatload mmc $mmcbootdev 0x81f00000 uRamdisk; bootm 0x81c00000 0x81f00000" \
	"\0" \
\
"autodetectmmc=if mmcinit 0; then\
 setenv mmcbootdev 0; \
else\
 setenv mmcbootdev 1; \
 fi" \
	"\0" \
\
"setserialnum=if fatload mmc ${mmcbootdev}:2 0x81c00000 devconf/DeviceID 100; then\
 setenvmem serialnum 0x81c00000 0x$filesize;\
fi" \
	"\0" \
\
"incbootcount=if fatload mmc ${mmcbootdev}:2 0x81c00000 devconf/BootCount 4; then\
 calc.l *0x81c00000 + 1 0x81c00000;\
 if itest.l *0x81c00000 > 99999; then\
  echo Boot count exceeded;\
  setenv bootvar recoveryboot;\
 fi;\
 fatsave mmc ${mmcbootdev}:2 0x81c00000 devconf/BootCount 4;\
fi" \
	"\0" \
\
...
if fatload mmc ${mmcbootdev}:2 0x81c00000 BCB 2000 &&\
