...
int do_fat_fsload (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (size == 0) {
		printf("\n\n############################\n");
		printf(    "# BIG WARNING - READ 0 BYTES\n");
		printf(    "############################\n\n");
	}
...
int do_fat_fssave (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	long size;
	unsigned long offset;
	unsigned long count;
	...
	if (argc < 5) {
		printf ("usage: fatsave <interface> <dev[:part]> <addr> <filename> [bytes]\n");
		return 1;
	}
	dev = (int)simple_strtoul (argv[2], &ep, 16);
	dev_desc=get_dev(argv[1],dev);
	...
	if (fat_register_device(dev_desc,part)!=0) {
		printf ("\n** Unable to use %s %d:%d for fatsave **\n",argv[1],dev,part);
		return 1;
	}
	offset = simple_strtoul (argv[3], NULL, 16);
	if (argc == 6)
		...
		count = 1; /* Default actually writes one complete sector */
	size = file_fat_write (argv[4], (const unsigned long *) offset, count);
...
U_BOOT_CMD(
	fatsave,	6,	0,	do_fat_fssave,
	"fatsave - save binary file to a dos filesystem\n",
	"<interface> <dev[:part]> <addr> <filename> [bytes]\n"
	"    - overwrite binary file 'filename' to 'dev' on 'interface'\n"
	"      from address 'addr' to dos filesystem. Whole sectors are\n"
	"      written, to a maximum of one cluster (defaults to 1 sector).\n"
);
