...
{
    if (--console_col < col_offset) {
        console_col = c_max_cols-1 ;
        if (--console_row < row_offset) {
            console_row = row_offset;
        }
    }
...
static inline void lcd_setpixel(ushort x, ushort y, ushort c)
{
...
static inline void lcd_drawchar(ushort x, ushort y, uchar c)
{
    ushort row, col, rx, ry;
...
void lcd_setprops(uchar orient, ushort xoff, ushort yoff)
{
	...
	/* Now compute the row and column offsets */
	row_offset = y_offset/(VIDEO_FONT_HEIGHT);
	col_offset = x_offset/(VIDEO_FONT_WIDTH);
