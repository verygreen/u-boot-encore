...
#endif
{
	unsigned int mmc_cont = CFG_ENV_MMC_CONT;
	...
	ret = mmc_read(mmc_cont, env_addr, (u_char *) env_ptr, CFG_ENV_SIZE);
	
	printf("Environment: %d bytes read: %s\n", CFG_ENV_SIZE, ret ? "OK" : "ERROR");

	if (crc32(0, env_ptr->data, CFG_ENV_SIZE) != env_ptr->crc)
		use_default = 1;

	if (use_default) {
		printf("Environment: Bad CRC, use default environment\n");		
		memcpy(env_ptr->data, default_environment, CFG_ENV_SIZE);
		env_ptr->crc = crc32(0, env_ptr->data, CFG_ENV_SIZE);
	} else {
		printf("Environment: Good CRC, use environment stored into the MMC\n");
	}
}
...
#endif
{
	...
	ret = mmc_write(mmc_cont, (u_char *) env_ptr, env_addr, len);
