...
int do_setenvmem ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	const char *addr = simple_strtoul(argv[2], NULL, 0);
	unsigned long len = simple_strtoul(argv[3], NULL, 0);
	...
	str[len] = 0;	/* ensure it's NUL-terminated */
	setenv(argv[1], str);
	free(str);
	return 0;
}
...
U_BOOT_CMD(
	setenvmem, CFG_MAXARGS, 3,	do_setenvmem,
	"setenvmem  - set environment variable from memory\n",
	"name addr length\n"
	"    - set environment variable 'name' to 'length'-byte string at 'addr'.\n"
);
