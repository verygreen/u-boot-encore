...
/**************************************************************************
 * get_gpmc0_type() - return which type of memory is plugged on GPMC 0 
 *    
 ***************************************************************************/
...
u32 get_board_rev(void)
{
    ...
    // board type gets populated board_init 
...
void display_board_info(u32 btype)
{
	...
	printf("OMAP%s-%s rev %d, CPU-OPP%s L3-%sMHz\n", cpu_s, sec_s, rev,
		p_cpu, p_l3);
	printf("OMAP3621-Boxer %s Version + %s (Boot %s)\n", db_s,
		mem_s, bootmode[2]);
#ifdef CONFIG_LED_INFO
