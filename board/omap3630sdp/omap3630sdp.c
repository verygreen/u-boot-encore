...
int board_init(void)
{
	...
	gpmc_init();	/* in SRAM or SDRAM, finish GPMC */
	 /* Linux mach id*/
	gd->bd->bi_arch_number = MACH_TYPE_OMAP_3630SDP;
	/* boot param addr */
...
void s_init(void)
{
	...
	if (cpu_is_3410()) {
		/* Lock down 6-ways in L2 cache,
		   so that effective size of L2 is 64K */
...
void set_muxconf_regs(void)
{
	...
	/* GPMC CS3 has LAN9211 device */
	MUX_VAL(CP(GPMC_nCS3), (IEN | PTU | EN  | M0))  /*GPMC_nCS3*/
	MUX_VAL(CP(GPMC_nCS7), (IEN | PTU | EN  | M4))  /*GPMC_58*/
	MUX_VAL(CP(McBSP1_DX), (IEN  | PTD | DIS | M4)) /*gpio_158 for LAN*/
