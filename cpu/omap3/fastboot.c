...
static int do_usb_req_set_address(void)
{
	...
	if (0xff == faddr) 
	{
		...
		if (*pwr & MUSB_POWER_HSMODE) {
			printf("High Speed mode detected\n");
			high_speed = 1;
		}
		else {
			printf("Low Speed mode detected\n");
			high_speed = 0;
		}
		ACK_REQ();
	}
	...
	if (*peri_rxcsr & MUSB_RXCSR_RXPKTRDY)
	{
		...
		if (0 == *rxcount) {
			...
			*peri_rxcsr &= ~MUSB_RXCSR_RXPKTRDY;
			...
			/* Get a buffer from the interface. If we don't get any buffer
			 * we use the fastboot_bulk_fifo buffer
			 */
			buffer = fastboot_interface->get_rx_buffer(&buffer_size);
			if(buffer == NULL) {
				buffer      = fastboot_bulk_fifo;
				buffer_size = fifo_size;
			}

			/* Check whether we have enough place into the buffer */
			nb_bytes = (buffer_size >=  count) ? count : buffer_size;
			...
			if (fifo_size == count) {
				...
				*peri_rxcsr |= MUSB_RXCSR_DMAENAB;
				...
				if (read_bulk_fifo_dma(buffer, nb_bytes)) {
					/* Failure */
					fastboot_rx_error();
				}
...
int fastboot_init(struct cmd_fastboot_interface *interface) 
{
	...
	device_strings[DEVICE_STRING_PRODUCT_INDEX]       = "3621EVT1A";
